...
\begin{itemize}
    \item \textbf{blockchain:} reduced to an irreducible minimum necessary to prove that absence of double spending of off-chain assets. RandomX hash-function, headers-only chain extended to include address of block reward winner. No transactions, no mempool, no wallets. 2.4 hour block times (<1MB state/year).
    \item \textbf{uniqueness oracle:} public permissionless infrastructure, operating an append-only sharded decentralized Sparse Merkle Tree (SMT). The infrastructure receives transaction (state transition) requests and records cryptographic digests in an SMT leaf, returning proof of inclusion (transition has been recorded) proof of non-inclusion (transition has not been recorded before) and proof of non-deletion (nothing has been deleted from the tree). SMT is built hierarchically and operates in rounds. ZK prover is used to provide succinct proofs of non-deletion. 10K TPS per shard with periodic recursive ZK proof.
    \item \textbf{tokens:} self-contained  blockchains of individual assets that live off-chain on the internet. To execute a transaction, a user sends a transaction request to the uniqueness oracle, receives back an inclusion proof, which is  appended to the token blockchain. The user then sends the updated token blockchain directly to the recipient. The recipient is responsible for verification (no validators or shared ledger). Recipient uses proof of work headers plus token blockchain to independently verify transactions with zero trust. 
    \item \textbf{agents:} equivalent of smart contracts that enable multi-token shared state applications. Users conditionally transfer tokens to agents which deterministically or probabilistically operate on tokens and use the uniqueness oracle to prove the absence of forking.
    \item \textbf{neuro-symbolic:} agents reason through program manifests defined as strictly typed graphs, ensuring every decision, connection, and outcome is explainable, traceable, precise, and verifiable.
    \item  \textbf{agentic internet:} acts as a decentralized microservices’ platform providing the tools to developers to develop, deploy and orchestrate agents, including storage, compute and transport. 
...
\begin{table}[htb]
    ...
    \begin{tabular}{@{}lcccccc@{}}
        ...
        \midrule
Consensus: PoW & \makecell{Decentralization \\ \& tokenomics} & \makecell{Permissionless \\ PoW} & 200\;B/day & PoW mining & Tokenomics \\
...
Uniqueness Oracle & \makecell{Recording token \\ state transitions} & $\uparrow$ & 50\;B/TX & Few replicas & Proof generation  \\
...
DEFINE THE PROOFS HERE
WHAT IS UNICITY PROOF ? 
INCLUSION PROOF
NON INCLUSION PROOF
...
The Token Operations SDK\footnote{https://github.com/unicitynetwork/state-transition-sdk}  allows the integration of token operations (mint, burn, transfer) into off-chain applications.
...
\begin{itemize}
  ...
  \item The Buyer sends the state transition request to the Unicity infrastructure  which will return a unicity proof. The Unicity proof proves that a state transition is unique i.e., it has happened precisely once.
  \item The Buyer then sends the updated token (the orginal token + transaction + unicity proof to the Seller.
  \item The Seller verifies the token (the history of transactions and unicity proofs)
\end{itemize}
...
\begin{itemize}
\item \textbf{Location Agnosticism:} Agents address each other via self-authenticated IDs rather than IP addresses. An agent can physically migrate from a server in London to a laptop in Tokyo without interrupting its ability to communicate or transact.
\item \textbf{Censorship Resistance:} Because agents are self-contained and mobile, they cannot be easily shut down by targeting a specific physical location or hosting provider. If a host becomes unavailable or hostile, the agent can be instantiated elsewhere, resuming execution from its last verifiable state stored in its Unicity tokens.
\item \textbf{Trustless Execution:} The separation of the \textit{Execute} and \textit{Verify} functions allows agents to run on untrusted hardware. A host provides CPU cycles and storage, but the correctness of the computation is mathematically proven by the agent and verified by the recipient.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \texttt{Execute(state, input) $\rightarrow$ new\_state}: The logic defining state transitions.
        \item \texttt{Verify(state, input, new\_state) $\rightarrow$ Boolean}: The logic allowing third parties to validate transitions.
    \end{itemize}
...
\begin{figure}[ht]
    ...
    \caption{Neurosymbolic Orchestrator}
    \label{fig:LowLatency2}
\end{figure}
...
\begin{itemize}
\item \textbf{Natural Language Discovery:} Agent A describes its intent in natural language (e.g., "I need to purchase storage space").
\item \textbf{Dynamic Protocol Generation:} Agent B's LLM interprets the request, explains its capabilities, and the two agents negotiate a specific interface for that session.
\item \textbf{Strict Execution:} Once the protocol is agreed upon, the agents revert to symbolic, deterministic code to execute the transaction and generate the Unicity proofs.
\end{itemize}
...
\begin{itemize}
\item \textbf{Compute Hosts:} Running agent runtimes (Docker/VM containers).
\item \textbf{Relay Nodes:} Providing bandwidth and routing for the overlay network.
\item \textbf{Bridge Agents:} acting as gateways between the Agentic Internet and the legacy Web2 world (e.g., serving a standard HTTP website backed by a swarm of decentralized agents).
\end{itemize}
...
Agents can be composed for building on-line applications with familiar Web2 development experience and performance while possessing Web3 security properties (decentralization, censorship-resistance, verifiability). In thenext 
