...
The internet is being rebuilt for machines: autonomous AI agents operating and transacting continuously. To enable agents to transact with the throughput, latency, and privacy required, we present Unicity, a new blockchain architecture that eliminates the shared ledger/validator construct of previous blockchain designs. On-chain state is reduced to the minimum necessary to prove uniqueness of off-chain transactions. Assets themselves are blockchains, self-authenticating cryptographic objects that move peer-to-peer without intermediary validation. This eliminates the fundamental bottlenecks of traditional blockchain designs: throughput becomes unconstrained, execution parallelizes without limit, and privacy is preserved by default. On this foundation, we construct the Autonomous Agentic Internet, a decentralized platform for the discovery, orchestration and execution of autonomous agents, with safety, explainability and verifiability by design.
...
\begin{itemize}
    ...
    \item \textbf{Tokens:}
    Verifiable state. Self-contained blockchains of individual assets that are minted directly off-chain or detached from other blockchains. To execute a transaction, a payor will send a transaction request to the uniqueness oracle and receive back an inclusion proof which is appended to the token. The payor then sends the updated token directly to the payee. Similar to physical cash in that only the payee is responsible for verification.
    \item \textbf{Verifiable Agents:}
 	Equivalent to traditional smart contracts enabling verifiable execution of multi-token applications such as decentralized exchange. Tokens are conditionally transferred to agents which execute and re-distribute tokens with proofs of execution and uniqueness. Agents are runtime-agnostic and independent, uniquely addressable computational entities that can migrate between environments. 
    \item \textbf{AI Agents:} Agents augmented with AI inference for intent interpretation, planning, and adaptive behavior. The verifiable execution environment ensures cryptographic verifiability while AI inference enables natural language interaction and autonomous decision-making. Probabilistic inference is constrained by strictly typed program graphs (neuro-symbolic logic) ensuring every decision, connection, and outcome is explainable, traceable, precise, and verifiable.
    \item \textbf{Agentic Internet:} A decentralized platform for autonomous agent operations. Agents can be developed, deployed, discovered, and orchestrated without centralized gatekeepers. Protocol agnostic P2P transport, Decentralized storage where agents carry state in tokens rather than host filesystems. Integrated micro-payments and infrastructure marketplace for compute, bandwidth, and proving services. \end{itemize}
...
AI agents are becoming autonomous economic actors. Today they operate within human workflows - writing code, executing trades, managing documents on command. The trajectory points toward full autonomy: agents that operate continuously, making decisions and taking actions without human oversight.
...
The first is transactional. Autonomous agents need infrastructure to transact and cooperate without trusted intermediaries. Traditional rails cannot serve them: banks require human authorization, payment processors reverse transactions, contracts require courts. Two autonomous agents cannot rely on reputation, legal recourse, or human judgment to resolve disputes. They need infrastructure where correctness is guaranteed cryptographically - where agreements execute according to mathematical rules rather than institutional permission.
...
What is missing is an architecture that addresses both challenges together. Transaction infrastructure that operates at machine scale without trusted intermediaries, combined with execution infrastructure that makes AI agent behavior cryptographically verifiable.
...
\textbf{Neuro-Symbolic Execution} solves the behavioral challenge. By separating neural interpretation from symbolic execution, Unicity constrains AI agent behavior to typed, deterministic operations. Neural components handle intent, parsing natural language, identifying relevant actions. Symbolic components handle execution, type-checked operations with cryptographic proofs of correctness. The result is AI agents whose decisions are explainable, whose actions are bounded to declared capabilities, and whose outputs carry cryptographic proof that execution followed the rules.
...
\textbf{Part II: Agentic Infrastructure} describes the computational entities that operate on this foundation. Tokens, Verifiable Agents, and AI Agents, along with platform services including the Neuro-Symbolic Orchestrator, peer-to-peer transport, decentralized storage, and micropayments.
...
This is true for all Layer 1 implementations. Bitcoin, Ethereum, Solana, Sui, Monad and every major blockchain maintain a single shared ledger. Each optimizes for different points across the decentralization-security-scalability tradeoff space, yet all remain constrained by the same fundamental architecture: a shared ledger managed by validators.

\medskip
Proposed optimizations reproduce this model at different scales. Layer 2 solutions move execution off-chain but route transactions through centralized sequencers who see, order and settle periodically to the L1. Sidechains create separate ledgers with their own validator sets. Sharding partitions state but maintains validators within each shard. 
\medskip
...
\begin{table}[ht]
    ...
    \begin{tabular}{@{}lllllll@{}}
        ...
        \midrule
        Consensus: PoW & \makecell[l]{Decentralization\\
        \& tokenomics} & \makecell[l]{Permissionless\\PoW} & 200 B/day & PoW mining & Tokenomics \\
...
\subsection{Mint: Chain Agnostic "Detached" Tokens}
...
Agents provide equivalent functionality in Unicity without global shared state. The underlying mechanism used is \textit{predicates}\footnote{For a detailed description of predicates along with privacy and security proofs see github.com/unicitynetwork/unicity-predicates-tex/releases/tag/latest}, generalized ownership conditions that define when a token can be transferred.
...
Using these primitives, a multi-party application operates as follows:

\begin{enumerate}
    \item \textbf{Lock:} Participants transfer their tokens to their local agents, attaching predicates that define the conditions under which the tokens may be released.
    
    \item \textbf{Execute:} The agents executes application logic over the locked tokens - matching orders, resolving game state, or computing payouts according to the application's rules.
...
\begin{enumerate}
    ...
    \item The output tokens include the execution proof, and the predicates ensure that settlement is atomic - both transfers succeed or neither does.
\end{enumerate}

This construction achieves several important properties:

\begin{itemize}
    \item \textbf{No custody risk:} The agent never has unilateral control over the tokens. Predicates enforce that tokens can only move according to the rules agreed upon by the participants.
    
    \item \textbf{No global order book:} Order state exists only within the locked tokens themselves. The agent matches orders as they arrive but does not maintain any persistent global state.
    
    \item \textbf{Privacy:} Only Alice, Bob, and the agent observe the trade. No validators witness the transaction, and no public ledger records the exchange.
    
    \item \textbf{Atomic settlement:} The interlocking predicates ensure that both sides of the trade settle together. There is no counterparty risk - neither party can be left holding worthless commitments.
    
    \item \textbf{Parallel execution:} Every trade is independent of every other trade. A thousand trading pairs can execute simultaneously without contention for shared resources.
\end{itemize}
...
\textbf{AI Agents} extend Verifiable Agents with AI inference. They combine cryptographic verifiability with neural capabilities for intent interpretation, planning, and adaptive behavior.
...
\begin{table}[h]
\centering
\caption{Verification mechanisms for Verifiable Executors}
\begin{tabular}{p{3cm}p{4.5cm}p{3cm}p{3cm}}
\hline
\textbf{Mechanism} & \textbf{How it works} & \textbf{Trust assumption} & \textbf{Cost} \\
\hline
...
\textbf{Validator committees} provide verification through consensus. A set of validators independently execute and attest to correctness. This reintroduces coordination overhead but may be appropriate for high-value transactions requiring additional assurance.
...
\begin{itemize}
    \item \textbf{Simple token transfers}: Re-execution. Verification is trivial.
    \item \textbf{Complex financial logic}: ZK proofs. Expensive computation, cheap verification.
    \item \textbf{High-frequency trading}: Fraud proofs. Minimize latency, penalize cheaters.
    \item \textbf{AI inference}: TEE attestation.
    \item \textbf{High-value settlements}: Validator committee. 
\end{itemize}
...
\begin{itemize}
    \item \textbf{True decentralization:} No validator set processes transactions. No sequencer orders them. Trades execute peer-to-peer between counterparties, coordinated by agents but settled directly.
    
    \item \textbf{Censorship resistance:} There is no mempool to monitor, no block producer to bribe, and no validator to pressure. A trade between two willing parties cannot be stopped by any third party.
    
    \item \textbf{Unlimited throughput:} Every trade is independent. There is no shared state to contend for, no block space to compete over. A million trades can settle simultaneously without interference.
    
    \item \textbf{Privacy by default:} Only the counterparties observe a trade. No public ledger records the exchange. No chain analysis can trace the flow of funds. Privacy is not an add-on requiring expensive cryptography - it is the default.
\end{itemize}
...
Agentic commerce - autonomous agents transacting for goods, services, and data, is projected to exceed \$3 trillion by 2030. Yet the infrastructure for agents to discover each other, negotiate terms, and settle transactions remains nascent. 
...
\begin{itemize}
    \item \textbf{Physical goods}: ``Purchase 100 units of component X, delivered to location Y, within 7 days, maximum price Z''
    \item \textbf{Services}: ``Require 1000 GPU-hours of compute, minimum spec A, availability 99.9\%, price range B-C'', `` I'm hungry, get me a pizza delivered", "I want to exchange my cash for crypto", "I'd like to watch the Manchester United game that starts in 10 minutes".
    \item \textbf{Data}: ``Seeking real-time feed of market data D, latency under 10ms, willing to pay E per hour''
    \item \textbf{Prediction}: ``I predict the NY Giants beat the Chicago Bears by 15 points''
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Initiation}: Either party initiates a dispute, posting a bond to prevent frivolous claims
    \item \textbf{Arbitrator Selection}: Parties select an arbitration agent from a registry, or use one specified in the original transaction terms
    \item \textbf{Evidence Submission}: Both parties submit evidence  - delivery confirmations, communication logs, sensor data, attestations from fulfillment agents
    \item \textbf{Deliberation}: The arbitration agent evaluates evidence against the original transaction terms
    \item \textbf{Ruling}: The arbitrator issues a signed ruling specifying escrow disposition
    \item \textbf{Enforcement}: The escrow agent executes the ruling automatically
\end{enumerate}
